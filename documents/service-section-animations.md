...
`ResponsiveServiceAnimation.jsx::ResponsiveServiceAnimation` renders a stable `ServiceAnimationShell` frame first, then observes each animation wrapper through a shared module-level `IntersectionObserver` with a `600px` vertical root margin. The desktop viewport check also uses one shared external subscription, so the four timeline steps do not each attach their own media-query listener or visibility observer. The dynamic animation module is imported only when its timeline step approaches the viewport, so the first home page view does not fetch the four service animation chunks while the section still preserves its desktop aspect ratio and scroll height.

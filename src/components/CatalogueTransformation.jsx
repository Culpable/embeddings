...
function ServiceStep({ number, label, className = '' }) {
  return (
    <div
      className={`relative z-10 grid grid-cols-[1.75rem_minmax(0,1fr)] items-center justify-start gap-x-3 rounded-full bg-white px-2 py-2 shadow-[0_1px_0_rgba(23,23,23,0.04)] ring-1 ring-neutral-950/5 ${className}`}
    >
      <span className="flex h-7 w-7 items-center justify-center rounded-full bg-neutral-950 text-xs font-semibold text-white">
        {number}
      </span>
      <span className="min-w-0 text-left text-sm font-medium text-neutral-950">
        {label}
      </span>
    </div>
  )
...
function ComparisonFact({ label, value, tone = 'neutral' }) {
  const valueClasses =
    tone === 'success'
      ? 'text-emerald-700'
      : tone === 'warning'
  ...
  return (
    <div className="rounded-2xl border border-neutral-950/10 bg-neutral-50 px-3 py-2">
      <dt className="text-[0.65rem] font-semibold uppercase tracking-widest text-neutral-400">
        {label}
      </dt>
      <dd className={`mt-1 text-sm font-semibold ${valueClasses}`}>{value}</dd>
    </div>
  )
}
...
function AttributeTag({ children, muted = false }) {
  return (
    <span
      className={
        muted
          ? 'before-shimmer inline-grid h-7 place-items-center rounded-full border border-dashed border-neutral-300 px-3 text-xs text-neutral-400'
          : 'inline-grid h-7 place-items-center rounded-full bg-neutral-950/[0.06] px-3 text-xs font-medium text-neutral-700'
      }
    >
      {children}
    </span>
  ...
  return (
    <div className="relative rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-8">
      ...
      <dl className="mt-5 grid grid-cols-2 gap-2">
        <ComparisonFact label="coverage" value="thin" tone="warning" />
        <ComparisonFact label="agent signal" value="unclear" tone="warning" />
      </dl>
      ...
      {/* Last updated — stale */}
      <p className="mt-5 text-xs text-red-500/80">Last updated: 8 months ago</p>
    </div>
  )
}
...
function AfterCard() {
  return (
    <div className="relative rounded-2xl border border-neutral-200 bg-white p-6 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl sm:p-8">
      ...
      <h4 className="mt-5 font-display text-lg font-medium text-neutral-950">
        Women&rsquo;s A-Line Midi Dress &mdash; Sapphire Blue, Taylor
        Swift-inspired
      </h4>
      ...
      <p className="mt-4 text-sm leading-relaxed text-neutral-600">
        Flattering A-line midi dress in sapphire blue crepe. Features a fitted
        bodice with subtle darting, a flowing midi-length skirt, and concealed
        side zip. Inspired by the blue dress trend popularised by Taylor Swift.
        Perfect for weddings, racing carnivals, and cocktail events. Machine
        washable. Available in sizes 6&ndash;18.
      </p>

      <dl className="mt-5 grid grid-cols-2 gap-2">
        <ComparisonFact label="coverage" value="complete" tone="success" />
        <ComparisonFact label="agent signal" value="clear" tone="success" />
      </dl>
      ...
      {/* Last updated — fresh */}
      <p className="mt-4 text-xs text-emerald-600">Last updated: 2 hours ago</p>
    </div>
  )
}
    ...
    <>
      <FadeInStagger>
        <div className="grid grid-cols-1 items-start gap-x-8 gap-y-10 lg:grid-cols-[1fr_auto_1fr]">
          ...
          <FadeIn>
            <div className="relative grid grid-cols-2 items-start gap-x-4 gap-y-3 rounded-3xl border border-neutral-950/10 bg-neutral-50 p-3 shadow-[0_1px_0_rgba(23,23,23,0.04)] md:w-full md:grid-cols-4 md:gap-x-4 md:gap-y-0 lg:flex lg:flex-col lg:items-start lg:gap-5 lg:border-0 lg:bg-transparent lg:p-0 lg:py-8 lg:shadow-none">
              {/* Arrow (horizontal on mobile, vertical on desktop) */}
              <div
                className="hidden h-6 w-px bg-gradient-to-b from-neutral-300 to-neutral-950/20 lg:block"
                aria-hidden="true"
              />
              ...
              <ServiceStep
                number="1"
                label="Audit"
                className="w-[8.75rem] justify-start justify-self-center md:w-full md:justify-start md:justify-self-stretch lg:w-full"
              />
              ...
              <div
                className="hidden h-6 w-px bg-gradient-to-b from-neutral-950/20 to-neutral-300 lg:block"
                aria-hidden="true"
              />
            </div>
          </FadeIn>

      ...
      <div className="relative overflow-hidden rounded-4xl bg-neutral-950 px-6 py-20 sm:px-12 sm:py-28 md:py-32">
        ...
        >
          {floatingSnippets.map(({ text, x, delay, duration, opacity }) => (
            <span
              key={text}
              className="floating-catalogue-snippet absolute whitespace-nowrap font-mono text-xs text-white"
              style={{
                '--snippet-x': x,
                '--snippet-delay': delay,
                '--snippet-duration': duration,
                '--snippet-opacity': Math.min(
                  Number(opacity) * snippetOpacityBoost,
                  maxSnippetOpacity,
                ),
              }}
            >
